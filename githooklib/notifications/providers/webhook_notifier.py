...
class WebhookNotifier(BaseNotifier):
    ...
    def send(
        ...
        if not self.enabled:
            logger.trace("Webhook notifier is disabled")
            return False
        ...
        logger.debug("Sending webhook notification for hook: %s", hook_name)
        ...
        try:
            ...
            if response.status_code in [200, 201, 202, 204]:
                ...
                logger.warning(
                    "Failed to send webhook notification: %d %s",
    ...
    def _send_via_curl(self, payload: dict) -> bool:
        ...
        try:
            ...
            if result.success:
                logger.debug("Webhook notification sent via curl")
                return True
            else:
