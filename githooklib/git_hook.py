...
class GitHook(ABC):
    ...
    def run(self) -> int:
        ...
        try:
            ...
            if not self._should_run_based_on_patterns(context):
                ...
                self._send_notification(hook_name, True, "Hook skipped (no matching files)")
                return EXIT_SUCCESS
            ...
            self._send_notification(hook_name, result.success, result.message)
            
            return result.exit_code
        except Exception as e:  # pylint: disable=broad-exception-caught
            self.logger.debug("Exception occurred during hook execution: %s", e)
            self._handle_error(e)
            self._send_notification(hook_name, False, f"Exception: {str(e)}")
            return EXIT_FAILURE
    ...
    def _send_notification(self, hook_name: str, success: bool, message: Optional[str] = None) -> None:
        try:
            ...
            if config.notifications.enabled:
                ...
                notification_service.notify(hook_name, success, message)
        except Exception as e:
            self.logger.trace("Failed to send notification: %s", e)
